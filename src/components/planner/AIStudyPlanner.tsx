      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="stitch-card rounded-2xl p-6 space-y-4">
          ...
          <form onSubmit={handleGenerate} className="space-y-4">
            <div>
              <label className="block text-xs text-neutral-400 mb-1">Target Course / Subject</label>
              <input
                type="text"
                value={course}
                onChange={e => setCourse(e.target.value)}
                className="w-full bg-[#121316] border border-white/10 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm text-neutral-200 focus:outline-none focus:border-violet-500"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-neutral-400 mb-1">Exam / Deadline Date</label>
                <input
                  type="date"
                  value={examDate}
              ...
              <div>
                <label className="block text-xs text-neutral-400 mb-1">Daily Available Study Runway (Hours)</label>
                <input
                  type="number"
                  step="0.5"
                  min="0.5"
            ...
            <div>
              <label className="block text-xs text-neutral-400 mb-1">Syllabus Modules or Lecture Topics</label>
              <textarea
                rows={3}
                value={syllabusNotes}
                onChange={e => setSyllabusNotes(e.target.value)}
                className="w-full bg-[#121316] border border-white/10 rounded-xl p-3 text-xs text-neutral-200 focus:outline-none focus:border-violet-500"
        ...
        <div className="stitch-card rounded-2xl p-6 flex flex-col justify-between border-violet-500/20 bg-gradient-to-b from-[#191a1f] to-[#131418]">
          <div>
            ...
            {aiOutput ? (
              <div className="bg-[#121316] border border-white/5 rounded-xl p-4 font-mono text-xs text-neutral-300 whitespace-pre-line leading-relaxed space-y-2 max-h-96 overflow-y-auto">
                {aiOutput}
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center py-16 text-center text-neutral-500 space-y-3">
                <Brain className="w-10 h-10 stroke-[1.5] text-neutral-600" />
          ...
          {aiOutput && (
            <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-xs text-neutral-400">Add to Circadian Task Board?</span>
              <button
                type="button"
